...
data_csv["scotese_lat"], data_csv["scotese_lon"] = zip(
    *data_csv.apply(
        lambda row: get_scotese_paleolocation(
            row["modern_lat"], row["modern_lon"], reconstruction_age
        ),
        axis=1,
    )
)
...
for idx, (ds_orog, ds_mask, orog_name, mask_name) in enumerate(data):
    ...
    # add sitye markers
    add_proxy_location_markers(
        ax=axes[idx], exp=exp_list[idx], data_csv=data_csv, size=8
    )
...
for idx, (ds_orog, ds_mask, orog_name, mask_name) in enumerate(data):
    ...
    # add sitye markers
    add_proxy_location_markers(
        ax=axes[idx], exp=exp_list[idx], data_csv=data_csv, size=10
    )
...
exp_labels = [
    ...
    "HadCM3+Scotese 560",
    "HadCM3+Scotese 1103",
    "HadCM3+Scotese+phys 780",
    "HadCM3+Scotese+phys 1103",
    "HadCM3+Getech 560",
    "HadCM3+Getech 1120",
]
...
for idx, exp in enumerate(exp_list):
    ...
    ds_temp = xr.open_dataset(
        f"{data_dir}/raw/model_clims/{exp}.clim.nc", decode_times=False
    ).squeeze()
    ds_mask = xr.open_dataset(f"{data_dir}/raw/model_clims/{exp}.mask.nc").squeeze()
...
for idx, (ds_temp, ds_mask, temp_name, mask_name) in enumerate(data):
    ...
    # add sitye markers
    add_proxy_location_markers(
        ax=axes[row, column], exp=exp_list[idx], data_csv=data_csv, size=10
    )
...
fig.suptitle(
    "Aptian Surface Air Temperature Anomalies", fontsize=16, fontweight="bold", y=0.95
)
...
if save_figures:
    plt.savefig(
        fig_dir + "/regional_aptian_temperature_anomaly_maps.pdf", bbox_inches="tight"
    )
...
for idx, exp in enumerate(exp_list):
    ...
    ds_pr = ds_pr.mean(time_name) * 86400.0
...
if save_figures:
    plt.savefig(
        fig_dir + "/regional_aptian_precipitation_maps.pdf", bbox_inches="tight"
    )
...
if save_figures:
    plt.savefig(
        fig_dir + "/regional_aptian_precipitation_anomaly_maps.pdf", bbox_inches="tight"
    )
...
for idx, exp in enumerate(exp_list):
    ...
    ds_orog = xr.open_dataset(
        f"{data_dir}/raw/model_clims/{exp}.orog.nc", decode_times=False
    ).squeeze()
    ...
    rotation = (
        "kcm"
        if exp in ["KCM_600", "KCM_1200"]
        else "scotese" if exp in ["texzx1", "texpx2", "tfkex", "tfksx"] else "getech"
    )
    plat = data_csv.loc[:, f"{rotation}_lat"]
    plon = data_csv.loc[:, f"{rotation}_lon"]
    ...
    for idxSite, row in data_csv.iterrows():
        site_info = {
            ...
            .sel(
                {lat_name_orog: plat[idxSite], lon_name_orog: plon[idxSite]},
                method="nearest",
            )
            .values.round(0),
        }
        ...
        for var_name, var_label in [
            ...
            values = (
                ds_clim[var_name]
                .sel(
                    {lat_name: plat[idxSite], lon_name: plon[idxSite]}, method="nearest"
                )
                .values
            )
            ...
            values_zonal = (
                ds_clim[var_name]
                .sel({lat_name: plat[idxSite]}, method="nearest")
                .values
            )
...
output_df.to_csv(
    f"{data_dir}/processed/simulated_temperature_precipitation_data_at_locations.csv",
    index=False,
)
...
def plot_proxy_mean_and_shading(ax, proxy_data, site_data, variable, label, color):
    ...
    # Shading the range
    ax.fill_between(
        months, [proxy_data[0]] * 12, [proxy_data[2]] * 12, color=color, alpha=0.2
    )
...
for idx, exp_pair in enumerate(model_pairs):
    # Load data
    ds_clim_1 = xr.open_dataset(
        f"{data_dir}/raw/model_clims/{model_pairs[exp_pair][0]}.clim.nc",
        decode_times=False,
    ).squeeze()
    ds_clim_2 = xr.open_dataset(
        f"{data_dir}/raw/model_clims/{model_pairs[exp_pair][1]}.clim.nc",
        decode_times=False,
    ).squeeze()
    ...
    rotation = (
        "kcm"
        if model_pairs[exp_pair][0] in ["KCM_600", "KCM_1200"]
        else (
            "scotese"
            if model_pairs[exp_pair][0] in ["texzx1", "texpx2", "tfkex", "tfksx"]
            else "getech"
        )
    )
    plat = data_csv.loc[:, f"{rotation}_lat"]
    ...
    temp_low_site_1 = (
        ds_clim_1[temp_name]
        .sel({lat_name: plat[0], lon_name: plon[0]}, method="nearest")
        .mean(time_name)
        .values
    )
    temp_high_site_1 = (
    ...
    temp_low_site_2 = (
        ds_clim_1[temp_name]
        .sel({lat_name: plat[1], lon_name: plon[1]}, method="nearest")
        .mean(time_name)
        .values
    )
    temp_high_site_2 = (
    ...
    axs.errorbar(
        np.mean([plat[0], plat[1]]),
        model_mean,
        yerr=np.array([[model_lower_error], [model_upper_error]]),
        fmt="o",
        color=colors[exp_pair],
        capsize=5,
        zorder=5,
    )
...
for idx, model in enumerate(exp_list):
    ...
    model_data_temp = filtered_df[
        (filtered_df["model"] == model) & (filtered_df["variable"] == "temperature")
    ]
    model_data_prec = filtered_df[
        (filtered_df["model"] == model) & (filtered_df["variable"] == "precipitation")
    ]

    # Site TSG - Temperature
    axs[0].scatter(
        model_data_temp[model_data_temp["site"] == "TSG"]["pheight"],
    ...
    axs[0].scatter(
        model_data_temp[model_data_temp["site"] == "SVO"]["pheight"],
    ...
    axs[1].scatter(
        model_data_prec[model_data_prec["site"] == "TSG"]["pheight"],
    ...
    axs[1].scatter(
        model_data_prec[model_data_prec["site"] == "SVO"]["pheight"],
...
average_prec_diff = (
    ...
    - filtered_df[(filtered_df["variable"] == "precipitation")][
        "annual_mean_zonal_mean"
    ]
)
...
axs[0].axhline(
    y=mean_temp_diff, color="black", linestyle="-", linewidth=3, label="Ensemble Mean"
)  # Add horizontal line
axs[1].axhline(
    y=mean_prec_diff, color="black", linestyle="-", linewidth=3, label="Ensemble Mean"
)  # Add horizontal line
